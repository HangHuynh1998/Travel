...
const getCategoryTours = (id, filter) => {
    ...
    if (filter.jobType) {
        query["tourType"] = filter.jobType;
    }
    if (filter.jobStatus) {
        query["status"] = filter.jobStatus
    }
    let pageIndex = filter.page ? Number(filter.page) - 1 : 0;
    let perPage = filter.limit ? Number(filter.limit) : 1000;
...
const editTour = (id, user, body) => {
    return new Promise(async (resolve, reject) => {
        try {
            let tour = await Job.findById(id);
            if (!tour) throw new Error("Tour not found !");
            if (user._id.toString() === job.company_id.toString()) {
                await updateDocument(job, Job, body, ["company_id"]);
                await job.save();
                return resolve(job);
            } else {
