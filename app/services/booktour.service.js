...
const { SEND_EMAIL_APPLY_JOB} = EMAIL_TYPE
const APP_DOMAIN = require("../../config/index").APP_DOMAIN;

const sendMailApplyTour = async (emailCompany,nameCompany,name,sex,birthday,address,emailCustomer,phone,title,tour_id,message) => {
    try {
        try {
            Mailer(
                `"Travel" <${gigZooAdminEmail}>`,
                emailCompany
            ).sendMail(SEND_EMAIL_APPLY_JOB, {
...
const addApplication = (customer_id, body) => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            if (!customer) {
                return reject({ message: "Only customer can apply to job" });
            } else {
                ...
                if (i === -1) {
                    ...
                    await tour.save();
                    // await Job.findByIdAndUpdate(body.job_id, { isApplied: job.isApplied.push(seeker_id) })
